...
public class GodManager {
    ...
    private void initializeGods() {
        ...
        // Register tutorial divine item
        registerDivineItem(GodType.TUTORIAL, new com.example.minecraftplugin.items.TutorialDivineItem());
        
        // Register the Convergence Nexus (not tied to a specific god)
        // This will be handled by the ConvergenceManager
        
        // Register standalone PvP items
        // Note: MaceOfAnnihilation is not tied to any specific god
        // It can be obtained through special means (raids, forge, etc.)
        ...
        logger.info("Note: MaceOfAnnihilation is available as a standalone PvP artifact");
    }
    ...
    public int getTestamentCount(Player player) {
        // Exclude tutorial god from testament count
        Set<GodType> completed = getCompletedTestaments(player);
        completed.remove(GodType.TUTORIAL);
    ...
    public boolean hasAchievedConvergence(Player player) {
        // Exclude tutorial god from convergence check
