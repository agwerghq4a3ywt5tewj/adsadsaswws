...
public class PlayerDataManager {
    ...
    private PlayerData loadPlayerDataFromConfig(UUID playerId) {
        ...
        // Load raid points
        int raidPoints = dataConfig.getInt(path + ".raid_points", 0);
        
        return new PlayerData(deathCount, isPrisonerOfVoid, pledgedGod, completedTestaments, 
                             collectedFragments, lastChestFragmentTime, lastMobFragmentTime, abilityCooldowns, raidPoints);
    }
    ...
    private void savePlayerDataToConfig(UUID playerId, PlayerData data) {
        ...
        // Save raid points
        dataConfig.set(path + ".raid_points", data.raidPoints);
    }
    ...
    private PlayerData getOrCreatePlayerData(UUID playerId) {
        return playerDataCache.computeIfAbsent(playerId, k -> new PlayerData(0, false, null, new HashSet<>(), new HashMap<>(), 0, 0, new HashMap<>(), 0));
    }
    ...
    /**
     * Get a player's raid points
     */
    public int getRaidPoints(Player player) {
        return getRaidPoints(player.getUniqueId());
    }
    
    public int getRaidPoints(UUID playerId) {
        return getOrCreatePlayerData(playerId).raidPoints;
    }
    
    /**
     * Set a player's raid points
     */
    public void setRaidPoints(Player player, int points) {
        setRaidPoints(player.getUniqueId(), points);
    }
    
    public void setRaidPoints(UUID playerId, int points) {
        PlayerData data = getOrCreatePlayerData(playerId);
        data.raidPoints = Math.max(0, points);
        savePlayerData(playerId);
    }
    
    /**
     * Add raid points to a player
     */
    public void addRaidPoints(Player player, int points) {
        addRaidPoints(player.getUniqueId(), points);
    }
    
    public void addRaidPoints(UUID playerId, int points) {
        PlayerData data = getOrCreatePlayerData(playerId);
        data.raidPoints += points;
        savePlayerData(playerId);
    }
    
    /**
     * Remove raid points from a player
     */
    public boolean removeRaidPoints(Player player, int points) {
        return removeRaidPoints(player.getUniqueId(), points);
    }
    
    public boolean removeRaidPoints(UUID playerId, int points) {
        PlayerData data = getOrCreatePlayerData(playerId);
        if (data.raidPoints >= points) {
            data.raidPoints -= points;
            savePlayerData(playerId);
            return true;
        }
    ...
    public String getPlayerDataSummary(Player player) {
        ...
        summary.append("\n§7Raid Points: §f").append(data.raidPoints);
