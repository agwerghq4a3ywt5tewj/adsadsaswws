...
public class RaidCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (subCommand) {
            ...
            case "test":
                if (args.length != 2) {
                    player.sendMessage("§cUsage: /raid test <raid_id> [player_count]");
                    return true;
                }
                if (player.hasPermission("minecraftplugin.admin")) {
                    String playerCountStr = args.length > 2 ? args[2] : "1";
                    handleTestRaid(player, args[1], playerCountStr);
                } else {
                    player.sendMessage(ChatColor.RED + "You don't have permission to test raids!");
                }
                break;
    ...
    private void showRaidHelp(Player player) {
        ...
        if (player.hasPermission("minecraftplugin.admin")) {
            ...
            player.sendMessage("§7/raid test <raid_id> [player_count] - Start test raid");
        }
    ...
    private void startRaid(Player player, String raidId) {
        ...
        if ("enhanced_ender_dragon".equals(raidId)) {
            ...
            players.add(player);
            Location raidLocation = player.getLocation();
            // Create default scaling for enhanced dragon combat
            com.example.minecraftplugin.raids.RaidScalingCalculator.RaidScaling scaling = 
                new com.example.minecraftplugin.raids.RaidScalingCalculator.RaidScaling(1.5, 1.2, 1.0, 1.0);
            
            player.sendMessage("§5§l=== SEKIRO-STYLE DRAGON COMBAT ===");
            player.sendMessage("§7This enhanced dragon features:");
            player.sendMessage("§7• §ePosture System §7- Parry attacks to break posture");
            player.sendMessage("§7• §cDeathblow Mechanics §7- Right-click when posture is broken");
            player.sendMessage("§7• §6Parryable Attacks §7- Yellow particles, time your strikes");
            player.sendMessage("§7• §cPerilous Attacks §7- Red particles, must dodge");
            player.sendMessage("§7• §dPhase Transitions §7- Dragon becomes more aggressive");
            player.sendMessage("§5§l=====================================");
            
            plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                plugin.getEnderDragonCombatManager().startCombat(null, players, scaling.getHealthMultiplier(), scaling.getDamageMultiplier());
    ...
    private void handleTestRaid(Player admin, String raidId, String playerCountStr) {
        int playerCount;
        try {
            playerCount = Integer.parseInt(playerCountStr);
            if (playerCount < 1 || playerCount > 10) {
                admin.sendMessage("§cPlayer count must be between 1 and 10");
                return;
            }
        ...
        testPlayers.add(admin);
        
        // Auto-fill with other online players if requested
        if (playerCount > 1) {
            for (Player onlinePlayer : plugin.getServer().getOnlinePlayers()) {
                if (onlinePlayer != admin && testPlayers.size() < playerCount) {
                    testPlayers.add(onlinePlayer);
                }
            }
        }
        
        admin.sendMessage("§a§lTest Raid Setup:");
        admin.sendMessage("§7Requested players: §f" + playerCount);
        admin.sendMessage("§7Actual players: §f" + testPlayers.size());
        admin.sendMessage("§7Participants: §f" + testPlayers.stream().map(Player::getName).reduce((a, b) -> a + ", " + b).orElse("None"));
        
        boolean success = raidManager.startTestRaid(raidId, testPlayers);
        if (!success) {
            admin.sendMessage("§c§lFailed to start test raid! §r§cCheck raid ID and try again.");
        }
    }
}