...
public class TestamentCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("proposal")) {
            if (args.length == 1) {
                showProposalHelp(player);
            } else {
     ...
     * Show proposal help
     */
    private void showProposalHelp(Player player) {
        player.sendMessage(ChatColor.YELLOW + "Usage: /testament proposal <option>");
        player.sendMessage(ChatColor.GRAY + "Available options: create, vote, list, info");
    }
    ...
    private void handleProposalSubcommand(Player player, String[] args) {
        // Implementation for handling proposal subcommands
        player.sendMessage(ChatColor.RED + "Proposal system not yet implemented.");
    }
    
    /**
     * Show dragon combat help
     */
    private void showDragonCombatHelp(Player player) {
        player.sendMessage("§5§l=== SEKIRO-STYLE DRAGON COMBAT ===§r");
        player.sendMessage("§7The Enhanced Ender Dragon features advanced combat mechanics:");
        player.sendMessage("");
        player.sendMessage("§e§lPosture System:");
        player.sendMessage("§7• Attack the dragon during §eparry windows§7 to deal posture damage");
        player.sendMessage("§7• When posture breaks, dragon becomes vulnerable for 5 seconds");
        player.sendMessage("");
        player.sendMessage("§c§lDeathblow Mechanics:");
        player.sendMessage("§7• Right-click the dragon when its posture is broken");
        player.sendMessage("§7• Deals massive damage (15-30% of max health)");
        player.sendMessage("");
        player.sendMessage("§6§lAttack Types:");
        player.sendMessage("§7• §eParryable§7: Yellow particles - time your attack to parry");
        player.sendMessage("§7• §cPerilous§7: Red particles - must dodge, cannot parry");
    }
}