...
import com.example.minecraftplugin.enums.GodTier;
...
public class AltarGenerator implements Listener {
    ...
    private int findSuitableY(World world, int x, int z) {
        // Start from a reasonable height and work down, with better logic
        int startY = Math.min(world.getMaxHeight() - 20, 120);
        int endY = Math.max(world.getMinHeight() + 10, 60);
        
        for (int y = startY; y >= endY; y--) {
            Location testLocation = new Location(world, x, y, z);
            
            // Check if this is a good surface level with more thorough validation
            if (isGoodSurfaceLevel(world, x, y, z)) {
                return y;
            }
        }
    ...
    /**
     * Check if a location is a good surface level for altar placement
     */
    private boolean isGoodSurfaceLevel(World world, int x, int y, int z) {
        // Check ground block
        Material groundBlock = world.getBlockAt(x, y - 1, z).getType();
        if (!groundBlock.isSolid() || groundBlock == Material.SAND || groundBlock == Material.GRAVEL) {
            return false;
        }
        
        // Check air space above (need 4 blocks of air for altar)
        for (int checkY = y; checkY <= y + 3; checkY++) {
            Material blockType = world.getBlockAt(x, checkY, z).getType();
        ...
        for (int dx = -2; dx <= 2; dx++) {
            for (int dz = -2; dz <= 2; dz++) {
                ...
                // Find ground level in surrounding area
                int surroundingGroundY = -1;
                for (int checkY = y + 2; checkY >= y - 2; checkY--) {
                    if (world.getBlockAt(checkX, checkY, checkZ).getType().isSolid()) {
                        surroundingGroundY = checkY;
                        break;
                    }
                }
                
                // If surrounding ground is too different in height, reject
                if (surroundingGroundY == -1 || Math.abs(surroundingGroundY - (y - 1)) > 2) {
                    return false;
                }
            }
        }
    ...
    private GodType selectGodForLocation(Location location) {
        ...
        for (GodType god : suitableGods) {
            int weight = god.getTier() == GodTier.CORE ? 3 : 1;
